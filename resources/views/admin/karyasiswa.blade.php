...
<head>
    <title>admin-studentworks</title>
    <!-- [Meta] -->
    <meta charset="utf-8">
                        ...
                        <div class="card-body">
                            <div class="container">
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-4">
                                    ...
                                    <a href="{{route('form.studentwork')}}" class="btn btn-primary d-flex text-align-center gap-2">
                                        <i class="fas fa-plus"></i>
                                        <span>Tambahkan Data</span>
                                    </a>
                                </div>
                                
                                <table id="tableStudentwork" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        @foreach ($posts as $index => $studentwork)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>
                                                    @if ($studentwork->image)
                                                        <img src="{{ asset('storage/'.$studentwork->image) }}" alt="Image" width="50">
                                                    @else
                                                ...
                                                <td>{{ $studentwork->description }}</td>
                                                <td>{{ $studentwork->author->name }}</td>
                                                <td>{{ \Carbon\Carbon::parse($studentwork->created_at)->format('F j, Y') }}
                                                </td>
                                                <td class="align-middle text-center">
                                                  <div class="d-inline-flex gap-1">
                                                      <a href="{{ route('studentwork.edit', $studentwork->id) }}"
                                                          class="btn btn-sm btn-outline-primary" title="Edit">
                                                          <i class="bi bi-pencil-square"></i>
                                                      </a>
                                              
                                                      <form action="{{ route('studentwork.destroy', $studentwork->id) }}"
                                                          method="POST"
...
<script>
    ...
    function myFunction() {
        ...
        table = document.getElementById("tableStudentwork");
